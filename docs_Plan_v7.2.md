# Plan v7.2: AITTK 全域 AI 辅助决策系统开发计划
...
| LLM 提供商 | enum（Local / GeminiFlash / GeminiPro / ClaudeHaiku / ClaudeSonnet） | Local |
| 云端 API Key（Gemini） | string | （空） |
| 云端 API Key（Claude） | string | （空） |
...
用户可在 UserConfig 中自行配置云端 LLM 提供商（Gemini / Claude）并填入个人 API Key，此时游戏状态数据将发送至相应云服务，**该行为由用户主动选择并承担相关风险**；API Key 仅存储于本地配置文件，不经由 AITTK 服务器中转。
...
| 模型 | 参数量 | 中文能力 | 推荐场景 |
|---|---|---|---|
| Qwen2.5-7B | 7B | ★★★★★ | 首选，战术复盘与非战斗域辅助 |
| DeepSeek-R1-7B | 7B | ★★★★★ | 推理增强备选 |
| Llama3-8B | 8B | ★★★☆☆ | 英文环境备选 |
| Phi-3-mini | 3.8B | ★★☆☆☆ | 低配硬件备选 |

**云端模型（可选增强，需用户配置 API Key）：**

| 模型 | 提供商 | 中文能力 | 推荐场景 |
|---|---|---|---|
| Gemini 2.0 Flash | Google | ★★★★★ | 云端轻量首选，低延迟，适合路线辅助 |
| Gemini 1.5 Pro | Google | ★★★★★ | 复杂事件/长上下文分析 |
| Claude 3.5 Haiku | Anthropic | ★★★★☆ | 云端轻量备选，快速事件决策辅助 |
| Claude 3.5 Sonnet | Anthropic | ★★★★★ | 深度策略分析，高质量自然语言解释 |
...
### 9.5 模型策略权衡：路线推荐与事件决策（v7.2 新增）

#### 9.5.1 背景

非战斗域（尤其是路线推荐与事件推荐）的 LLM 辅助面临不同挑战：

- **路线推荐**：输入高度结构化（节点类型、层数、前瞻路径），但需要对未来局势做综合判断
- **事件推荐**：已收录事件可由规则引擎处理，未收录事件需要 LLM 理解自然语言描述，复杂度远高于路线推荐

这两个域对 LLM 的依赖程度不同，适合的模型规模也不同。

#### 9.5.2 本地 LLM（Ollama）与云端大模型（Gemini / Claude）综合对比

| 维度 | 本地 LLM（Ollama） | Gemini / Claude（云端） |
|---|---|---|
| 推理能力 | 中等（7B–13B 模型） | 强（数百亿至数千亿参数） |
| 中文理解（Qwen2.5 / Claude） | ★★★★★ / N/A | ★★★★★ |
| 响应延迟 | 取决于本地硬件（2–10 s） | 网络 + 服务延迟（0.5–3 s） |
| 隐私保护 | ✅ 全本地，无数据外泄 | ⚠️ 游戏数据发送至云端 |
| 使用成本 | ✅ 免费（仅本地算力） | 💰 按 Token 计费 |
| 离线可用 | ✅ 完全支持 | ❌ 需要网络连接 |
| 上下文长度 | 4 K–32 K（视模型而定） | 128 K–1 M（Gemini Pro） |
| 对未收录事件的处理能力 | 中等（依赖模型规模） | 强（更好的泛化推理） |
| 配置复杂度 | 需本地安装 Ollama | 需申请 API Key |

**核心结论：** 本地 LLM 满足隐私和离线需求，对于结构化程度高的路线推荐已足够；云端模型对未收录的复杂事件有更强的泛化能力，作为可选增强供有需求的用户启用。

#### 9.5.3 路线推荐的模型策略

**推荐：本地 LLM 为主，云端可选**

路线推荐的输入完全结构化，规则引擎已能覆盖绝大多数决策场景：

- 当前坐标与可选下一节点的类型（精英 / 宝箱 / 休息点 / 商店）
- 前瞻 2–3 层的节点分布
- RunContext（HP、牌组强度、金币、Boss 距离等）

LLM 在此域的主要价值是**自然语言解释**（"为什么选这条路"），而非决策本身。Prompt 输入可精简至 400 Token 以内，本地 Qwen2.5-7B 的输出质量可接受。

| 场景 | 推荐模型 |
|---|---|
| 标准路线解释（HP 充足，无特殊状态） | 本地 LLM（Qwen2.5-7B） |
| Boss 前最后几层的复杂取舍 | 本地 LLM 即可；偏好更丰富解释可用 Gemini Flash |
| 用户希望获得深度路线分析 | Gemini 1.5 Pro 或 Claude Sonnet（可选） |

**执行建议：** 路线推荐的 LLM Prompt 精简至 ≤ 400 Token；默认使用本地 Ollama；云端选项作为 UserConfig 可选项，不进入默认路径。

#### 9.5.4 事件决策的模型策略

**推荐：已收录事件用规则引擎，未收录事件优先本地 LLM，云端作为可选高级选项**

事件推荐的三层架构（事实 JSON + 策略 C# + LLM 兜底）已将 LLM 的必要性降至最低：

- **大多数核心事件**：通过 `EventEffects.json` + `EventOptionEvaluator.cs` 处理，无需 LLM
- **未收录事件**：需要 LLM 理解选项的自然语言描述，推断效果并结合 RunContext 给出建议

未收录事件场景的模型选择：

| 场景 | 推荐模型 | 理由 |
|---|---|---|
| 未收录事件，选项有明确数值描述 | 本地 LLM（Qwen2.5-7B） | 结构化程度高，本地模型可理解 |
| 未收录事件，效果涉及概率 / 条件分支 | Gemini 2.0 Flash 或 Claude 3.5 Haiku | 推理能力要求更高，云端轻量模型合适 |
| 用户希望获得深度事件策略分析 | Gemini 1.5 Pro 或 Claude 3.5 Sonnet | 大参数模型提供更丰富的策略解释 |

事件推荐的 LLM Prompt 允许稍长的上下文（≤ 800 Token），以充分描述事件选项的复杂效果和当前 RunContext。

#### 9.5.5 v7.2 执行建议汇总

| 建议项 | 说明 |
|---|---|
| **默认保持本地** | 所有 LLM 调用默认走本地 Ollama（`localhost:11434`），隐私优先，无需任何额外配置 |
| **可选云端增强** | 在 UserConfig 新增 `LlmProvider` 枚举（见 §8.3），用户按需配置 API Key |
| **分域差异化 Prompt** | 路线推荐 Prompt ≤ 400 Token；事件兜底 Prompt ≤ 800 Token |
| **云端不进入战斗域** | 任何云端 LLM 均不得接入战斗内实时推荐主链路 |
| **降级保证** | 无论本地还是云端，超时或不可用时均回退到规则引擎结果，不影响用户体验 |
| **API Key 本地存储** | 云端 API Key 仅存储于本地配置文件，不上传任何服务器 |
...
| # | 变更 | 位置 |
|---|---|---|
| 1 | 新增 §9.5：路线推荐与事件决策的模型策略权衡（本地 LLM vs Gemini / Claude） | §9.5 |
| 2 | 模型选型表扩展为本地模型（Ollama）+ 云端模型（Gemini / Claude）两组 | §9.3 |
| 3 | 路线推荐明确采用本地 LLM 为主、云端可选策略；Prompt Token 预算 ≤ 400 | §9.5.3 |
| 4 | 事件决策按收录/未收录情况分层推荐模型；复杂事件引入 Gemini Flash / Claude Haiku | §9.5.4 |
| 5 | 隐私声明更新：说明云端选项为用户主动启用，API Key 仅本地存储 | §9.2 |
| 6 | UserConfig 新增 LlmProvider 枚举与云端 API Key 配置项 | §8.3 |
| 7 | v7.2 执行建议汇总表（默认本地、分域 Prompt 预算、降级保证等） | §9.5.5 |
